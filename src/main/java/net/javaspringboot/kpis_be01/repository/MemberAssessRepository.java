...
public interface MemberAssessRepository extends JpaRepository<MemberAssessment,Long> {
    ...
    @Query(value = "select * from member_assess where unique_username = :username and created_at = :date",nativeQuery = true)
    List<MemberAssessment> findListMemberAssessUserDate(String username, String date);
